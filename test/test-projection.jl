@testset "Projection Functions" begin
    ...
    c = [0.0, 0.0]
    r = 1.0
    @test isapprox(NonSmoothDynamics.project_ball([2.0, 2.0], c, r), [0.707107, 0.707107], atol=1e-6)
    ...
    @test isapprox(NonSmoothDynamics.project_simplex([0.5, 0.5, 0.5]), [1/3, 1/3, 1/3], atol=1e-6)
    @test isapprox(NonSmoothDynamics.project_simplex([1.0, 0.5, 0.0]), [0.75, 0.25, 0.0], atol=1e-6)
    # NonSmoothDynamics.plot_simplex_projection([0.5, 0.5, 0.5])

    # Test projection onto an L2 norm ball
    @test isapprox(NonSmoothDynamics.project_l2_ball([3.0, 4.0], 5.0), [3.0, 4.0], atol=1e-6)
    @test isapprox(NonSmoothDynamics.project_l2_ball([6.0, 8.0], 5.0), [3.0, 4.0], atol=1e-6)

    # Test projection onto the positive orthant
    @test isapprox(NonSmoothDynamics.project_positive_orthant([-1.0, 2.0]), [0.0, 2.0], atol=1e-6)
...
plot!([triangle_x..., triangle_x[1]], [triangle_y..., triangle_y[1]], label="Simplex", color=:green, lw=2)
scatter!(x_vals, y_vals, label="Iterates", color=:red, markersize=4)
...
plot!(aspect_ratio=:equal, xlabel="x₁", ylabel="x₂", title="Projection onto Box ∩ Simplex using Boyle-Dykstra")
